...
if (kukSearchInput) {
    if (kukSearchInput.value !== '') {
        window.setTimeout(() => {
            kukSearchInput.focus();
            kukSearchInput.setSelectionRange(kukSearchInput.value.length, kukSearchInput.value.length);
        }, 0);
    }

    kukSearchInput.addEventListener('input', () => {
        window.clearTimeout(kukSearchTimer);
        kukSearchTimer = window.setTimeout(submitKukSearch, 450);
    });
}
